...
class VideoPipeline:
    ...
    def run_in_background(self, job_id: str, script: str, config: dict):
        thread = threading.Thread(
            target=self._safe_run, args=(job_id, script, config), daemon=True
        )
        thread.start()
        return thread
    ...
    def _execute(self, job_id, script, config):
        ...
        p("sfx", 1, 1, f"✅ {len([v for v in sfx_clips.values() if v])} SFX loaded")
            ...
            if voiceover_path:
                try:
                    ...
                    captions = transcriber.generate_captions(
                        {"segments": [{"start": s["id"] * 5, "end": s["id"] * 5 + s["duration"],
                                       "text": s["text"]} for s in scenes]},
                        style="standard"
                    )
        ...
        for i, size in enumerate(sizes):
            ...
            out = composer.compose(
                job_id=job_id, analysis=analysis, broll_clips=broll_clips,
                voiceover_path=voiceover_path, music_path=music_path,
                sfx_clips=sfx_clips, captions=captions, config=compose_cfg,
                progress_callback=lambda **kw: p(**kw)
            )
            if out:
                outputs.append({
